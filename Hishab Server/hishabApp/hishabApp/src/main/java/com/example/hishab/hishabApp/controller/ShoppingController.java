...
public class ShoppingController {
	...
	public ShoppingResponse addShopping(@RequestBody ShoppingRequest incomingShopping) {
		ShoppingResponse shoppingResponse = new ShoppingResponse(incomingShopping.getShoppingId(),incomingShopping.getLocalId(),new ArrayList<ShoppingItemResponse>());
	...
	private ShoppingItemResponse shoppingItemToShoppingItemResponse(ShoppingItemRequest shoppingItemRequest,ShoppingItem shoppingItem) {
		CategoryResponse categoryResponse = new CategoryResponse(shoppingItemRequest.getCategoryLocalId(),shoppingItem.product.getCategoryId());
		ProductResponse productResponse = new ProductResponse(shoppingItem.product.getProductName(),shoppingItem.product.getProductId(),shoppingItemRequest.getProductLocalId(),categoryResponse);
		...
		if (item.getProductRequest().getProductId() == -1) {
			if (item.getProductRequest().categoryRequest != null && item.getProductRequest().categoryRequest.getNewCategoryName() != "") {
				ProductResponse productResponse = productController.addOrUpdateProduct(new ProductRequest(-1,item.getProductRequest().categoryRequest,
						item.getProductRequest().getProductName(),insertedShopping.userModel.getUserId(), item.getProductRequest().getLocalId())); 
	...
	ShoppingResponse updateShoppingItem(@RequestBody ShoppingRequest incomingShopping) {
		...
		ShoppingResponse shoppingResponse = new ShoppingResponse(incomingShopping.getShoppingId(),incomingShopping.getLocalId(),
				new ArrayList<ShoppingItemResponse>());
		List<Integer> deleteList = new ArrayList<>();
		UserModel userModel  = userRepository.findUserModelByUserId(incomingShopping.getUserId());
		Shopping shopping = shoppingRepository.save(new Shopping(incomingShopping.getDate(),userModel,incomingShopping.getShoppingId()));
		incomingShopping.getShoppingItemRequests().forEach(item -> {
