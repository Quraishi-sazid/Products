...
/*Select c.category_id as category_id,c.category_name as category_name,cd.month,cd.year,sum(p.cost) as cost from category c left join product_table pr on c.category_id=pr.category_id
left join tbl_shopping_item p on pr.product_id=p.product_id
left join tbl_shopping s on p.shopping_id=s.shopping_id
left join custom_date cd on s.date_id=cd.date_id
group by c.category_id,cd.month,cd.year*/
