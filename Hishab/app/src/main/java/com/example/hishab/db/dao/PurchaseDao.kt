...
interface PurchaseDao {
    ...
    @Query("select c.category_id as CategoryId,c.category_name as CategoryName,sum(p.cost) as Cost from tbl_shopping_item p inner join product_table s on p.purchase_id=s.product_id inner join category c on s.category_id=c.category_id inner join tbl_shopping b on p.shopping_id=b.shopping_id inner join custom_date d on b.date_id=d.date_id where d.day>=:day and d.month>=:month and d.year>=:year group by c.category_id")
    suspend fun getTotalCostByCategoryFromDate(
        day: Int,
        month: Int,
        year: Int
